...
for path in pages:
    ...
    if 'data-vmg-trade-nav="true"' not in text:
        ...
        text = re.sub(
            r'<link rel="stylesheet" href="/assets/css/vmg-header-sticky-fix\.css(?:\?v=[^"]+)?" data-vmg-header-sticky-fix="true">',
            '<link rel="stylesheet" href="/assets/css/vmg-header-sticky-fix.css?v=20260822c" data-vmg-header-sticky-fix="true">',
            text,
            count=1,
        )
    ...
    text = re.sub(r'(?:/)?assets/js/config\.js(?:\?v=[^"\']+)?', '/assets/js/config.js?v=20260822c', text)
...
for item in changed:
    print(' -', item)
print('Hardcoded VMG header/footer refreshed from includes.')
