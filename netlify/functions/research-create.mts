...
export default async (req:Request,_ctx:Context)=>{
  ...
  try{
    ...
    const rows=await insert("research_jobs",{workspace_id:ws.id,company_id:company.id,template_key:template,custom_prompt:body.custom_prompt||null,input_seed:{seed:body.seed||"",confirmed_entity:entity},provider:route.provider+":"+route.model,usage_day:day,is_full_research:template!=="quick_company_check",status:"QUEUED"});
    ...
    const invoke=await fetch(new URL("/.netlify/functions/research-run-background",req.url),{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({job_id:job.id})});
    if(!invoke.ok){
    ...
    return json({job_id:job.id,company_id:company.id,status:"QUEUED",background_accepted:true},202);
