...
import { json, readJson, safeError, workspace, select, insert, STAGES, dayKey, config as appConfig, providerSecret } from "./lib.mts";
...
export default async (req:Request,_ctx:Context)=>{
  ...
  try{
    ...
    const gemini=await providerSecret("gemini");if(!gemini)return json({error:"Gemini is not connected. Open Settings → Research Providers to connect it.",code:"PROVIDER_NOT_CONFIGURED"},503);
