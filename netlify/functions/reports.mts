...
export default async (req:Request,_ctx:Context)=>{
  try{
    ...
    if(id){
      ...
      const ev=await select("evidence_items",`report_id=eq.${id}&select=*`);
      const src=await select("sources",`company_id=eq.${r[0].company_id}&select=*&order=retrieved_at.desc`);
      const c=await select("companies",`id=eq.${r[0].company_id}&select=id,legal_name,brand,country,state_region,website,updated_at&limit=1`);
      return json({report:r[0],company:c?.[0]||null,evidence:ev||[],sources:src||[]});
    }
