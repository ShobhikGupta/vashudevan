...
export default async (_req:Request,_ctx:Context)=>{
  ...
  return json({
    ...
    google_search_grounding:{configured:gs.configured,available:gs.connected&&by.gemini?.provider_metadata?.grounding_verified!==false,status:gs.connected?"AVAILABLE":gs.configured?"CONFIGURED":"NOT_CONFIGURED",role:"PRIMARY SEARCH",last_verified_at:by.gemini?.last_verified_at||null},
