...
import { json, safeError, select, insert, exportReport, workspace, workspaceSettings } from "./lib.mts";
...
export default async (req:Request,_ctx:Context)=>{
  try{
    ...
    const ws=await workspace(),rows=await select("research_reports",`workspace_id=eq.${ws.id}&id=eq.${encodeURIComponent(id)}&select=*&limit=1`);if(!rows?.length)return json({error:"Report not found"},404);
    ...
    await insert("exports",{workspace_id:ws.id,company_id:report.company_id,report_id:report.id,export_type:type,storage_path:null},false);
    await insert("activity_logs",{workspace_id:ws.id,action:"report_exported",company_id:report.company_id,research_job_id:report.research_job_id||null,metadata:{report_id:report.id,version_no:report.version_no,export_type:type}},false);
