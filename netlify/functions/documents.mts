...
export default async (req:Request,_ctx:Context)=>{
  try{
    ...
    const rows=await select("attachments",`workspace_id=eq.${ws.id}&company_id=eq.${encodeURIComponent(companyId)}&select=id,filename,mime_type,size_bytes,public_private,external_ai_allowed,upload_status,parse_status,extraction_metadata,uploaded_at,parsed_at,parse_error,created_at,research_job_id&order=created_at.desc`);
