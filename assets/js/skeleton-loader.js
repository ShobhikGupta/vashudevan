...
(function () {
  ...
  function normalizeContactRouting() {
    document.querySelectorAll('a[href]').forEach(function (link) {
      ...
      var isContactDestination = CONTACT_PATH_RE.test(url.pathname) ||
  ...
  function syncCountryTicker() {
    ...
    var source = groups.find(function (group) {
      return group.getAttribute('aria-hidden') !== 'true' && !group.classList.contains('vmg-country-ticker-clone');
    }) || groups[0];
    var clone = groups.find(function (group) {
      return group !== source && (group.classList.contains('vmg-country-ticker-clone') || group.getAttribute('aria-hidden') === 'true');
    });
    groups.forEach(function (group) { if (group !== source && group !== clone) group.remove(); });
    if (!clone) { clone = source.cloneNode(true); track.appendChild(clone); }
    source.removeAttribute('aria-hidden');
    source.classList.remove('vmg-country-ticker-clone');
    clone.classList.add('vmg-country-ticker-clone');
    clone.setAttribute('aria-hidden', 'true');
    if (!measureCountryTicker(track, source, clone)) window.setTimeout(scheduleCountryTickerSync, 120);
  }
  ...
  function guardHomepageTickerInstance() {
    ...
    if (!host) return;
    var embedScripts = Array.prototype.slice.call(host.querySelectorAll('script[src*="embed-widget-ticker-tape.js"]'));
    ...
    var frames = Array.prototype.slice.call(widgetRoot.children).filter(function (child) { return child.tagName === 'IFRAME'; });
  ...
  function ensureContactPeople() {
    ...
    var heading = headings.find(function (item) { return item.textContent.trim().toLowerCase() === 'contact details'; });
    if (!heading) return;
    ...
    document.querySelectorAll('.site-header .vmg-feedback-trigger').forEach(function (button) {
      ...
      button.addEventListener('click', function () {
        function openExistingDrawer() {
          ...
          document.querySelectorAll('.vmg-feedback-trigger').forEach(function (item) { item.setAttribute('aria-expanded', 'true'); });
          window.setTimeout(function () { var first = drawer.querySelector('.vmg-feedback-rating'); if (first) first.focus(); }, 20);
        }
        if (window.innerWidth <= 991) {
          var nav = document.getElementById('site-nav');
          var toggle = document.querySelector('.nav-toggle');
          if (nav && nav.classList.contains('open') && toggle) {
            toggle.click(); window.setTimeout(openExistingDrawer, 170); return;
          }
        }
        openExistingDrawer();
      });
  ...
  function scheduleFeedbackBinding() {
    ...
    var timer = window.setInterval(function () {
      attempts += 1;
      if (bindExistingFeedbackTriggers() || attempts >= 20) window.clearInterval(timer);
    }, 100);
  }

  function stabilizeTradingViewHosts() {
    var hosts = document.querySelectorAll('.market-ticker-tradingview, .market-widget-panel, .market-chart-widget');
    if (!hosts.length) return;
    var attempts = 0;
    function check() {
      attempts += 1;
      var measurable = Array.prototype.every.call(hosts, function (host) {
        var rect = host.getBoundingClientRect(); return rect.width > 0 && rect.height >= 0;
      });
      if (measurable || attempts >= 8) { window.dispatchEvent(new Event('resize')); return; }
  ...
  function syncFloatingStack() {
    ...
    if (height > 0) document.documentElement.style.setProperty('--vmg-help-trigger-height', height + 'px');
    return height > 0;
  }
  ...
  function scheduleFloatingStack() {
    ...
    function measure() {
      attempts += 1;
      if (!syncFloatingStack() && attempts < 24) window.setTimeout(measure, 100);
    }
    measure();
  ...
  function init() {
    ...
    document.addEventListener('vmg:loader-hidden', function () {
      scheduleCountryTickerSync(); guardHomepageTickerInstance(); stabilizeTradingViewHosts(); syncFloatingStack();
    });
    if (document.fonts && document.fonts.ready) document.fonts.ready.then(scheduleCountryTickerSync).catch(function () {});
